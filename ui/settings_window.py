...
class SettingsEditor(tk.Toplevel):
    ...
    def __init__(self, parent, on_save_callback=None):
        ...
        # Footer Buttons
        btn_frame = tk.Frame(self, pady=15, bg="#f0f0f0")
        btn_frame.pack(fill=tk.X, side=tk.BOTTOM)

        tk.Button(btn_frame, text="Save All Settings", bg="#4caf50", fg="white", font=("Arial", 10, "bold"),
    ...
    def _load_plugins(self):
        """
        Loads the extractor and validator plugins to populate the settings tabs.
        """
    ...
    def _init_plugin_vars(self, plugins):
        """
        Creates Tkinter variables for each configuration option in the plugins.
        """
    ...
    def _init_global_settings(self):
        """
        Builds the 'Global' tab with core system settings and the new Debug controls.
        """
        tab = tk.Frame(self.notebook, padx=10, pady=10)
        self.notebook.add(tab, text="Global")
        ...
        tk.Label(tab, text="Core System Settings", font=("Helvetica", 12, "bold")).pack(pady=(0, 15))

        # --- Extractor Selection ---
        lf_extractor = tk.LabelFrame(tab, text="Extraction Engine (Artifact 1)", padx=10, pady=10)
        lf_extractor.pack(fill=tk.X, pady=5)
        tk.Label(lf_extractor, text="Active Extractor Plugin:", width=25, anchor="w").pack(side=tk.LEFT)
        extractor_options = [e.name for e in self.extractors] if self.extractors else ["No Extractors Found"]
        extractor_combo = ttk.Combobox(lf_extractor, textvariable=self.active_extractor,
                                       values=extractor_options, state="readonly", width=35)
        extractor_combo.pack(side=tk.LEFT, padx=5)
        ...
        chk_debug = tk.Checkbutton(f_debug, text="Debug Mode (Verbose Logging)", variable=self.debug_mode)
        chk_debug.pack(side=tk.LEFT)

        # NEW: Button to reload plugins and print debug info immediately
        btn_reload = tk.Button(f_debug, text="Apply & Test Debug Output",
                               command=self.perform_debug_reload, bg="#e0e0e0", font=("Arial", 8))
        btn_reload.pack(side=tk.LEFT, padx=20)

        tk.Checkbutton(lf_toggles, text="Adaptive Ordering", variable=self.adaptive_ordering).pack(anchor="w")
        tk.Checkbutton(lf_toggles, text="Enable LLM / Deep Research", variable=self.llm_enabled).pack(anchor="w")

        # --- Thresholds ---
        lf_thresh = tk.LabelFrame(tab, text="Validation Logic (Artifact 2)", padx=10, pady=10)
        lf_thresh.pack(fill=tk.X, pady=5)
        self._add_spinbox(lf_thresh, "Satisfied Threshold (Count):", self.satisfied_threshold, 1, 10)
        self._add_spinbox(lf_thresh, "LLM Confidence Threshold (%):", self.llm_conf_threshold, 0, 100)
        ...
        tk.Label(f_split, text="Report Structure:", width=25, anchor="w").pack(side=tk.LEFT)
        split_combo = ttk.Combobox(f_split, textvariable=self.report_split_mode,
                                   values=["merged", "split", "both"], state="readonly", width=15)
        split_combo.pack(side=tk.LEFT)

    def perform_debug_reload(self):
        """
        Temporarily sets the global config.DEBUG_MODE to the value currently
        selected in the UI (even if not saved) and re-instantiates the plugin
        managers. This forces the plugins to re-load their settings, triggering
        the print statements in `_load_settings_from_file`.
        """
        original_debug = config.DEBUG_MODE

        # 1. Apply UI choice temporarily
        ...
        try:
            ...
            # Instantiating the manager triggers plugin loading
            ExtractionManager()
            ...
            print("\n[Success] Reload complete. Check console logs above for config details.")
        except Exception as e:
            print(f"\n[Error] Reload failed: {e}")
        finally:
            print("=" * 50 + "\n")
            # We intentionally leave config.DEBUG_MODE modified so repeated clicks
            # work as expected. It will be finalized when "Save All Settings" is clicked
            # or reverted if "Cancel" is clicked (since Cancel destroys the window
            # without saving to file, though the runtime config variable might remain
            # changed until restart if not carefully handled. Ideally, saving is required).

    def _init_category_tab(self, category_name, plugins):
        """
        Generic method to create a tab for a list of plugins (Extractors or Validators).
        """
        tab = tk.Frame(self.notebook, padx=10, pady=10)
        ...
        nav_frame = tk.Frame(tab, bg="#e1e1e1", padx=10, pady=10, relief=tk.RAISED, borderwidth=1)
        nav_frame.pack(fill=tk.X, pady=(0, 15))
        tk.Label(nav_frame, text=f"Select {category_name[:-1]}:", bg="#e1e1e1", font=("Arial", 10, "bold")).pack(
            side=tk.LEFT)

        plugin_names = [p.name for p in plugins]
        combo = ttk.Combobox(nav_frame, values=plugin_names, state="readonly", width=40)
        ...
        combo.bind("<<ComboboxSelected>>",
                   lambda e: self._render_plugin_settings(container_frame, combo.get(), plugins))
    ...
    def _render_plugin_settings(self, container, selected_name, plugins):
        """
        Renders the settings inputs for a specific selected plugin.
        """
        ...
        tk.Label(header_frame, text=f"Configuration: {plugin.name}", font=("Helvetica", 11, "bold")).pack(anchor="w")
        tk.Frame(header_frame, height=2, bd=1, relief=tk.SUNKEN).pack(fill=tk.X, pady=5)
        ...
        for key in sorted(vars_map.keys()):
            ...
            tk.Label(row_frame, text=f"{clean_key}:", width=35, anchor="w").pack(side=tk.LEFT)

            if isinstance(var, tk.BooleanVar):
                tk.Checkbutton(row_frame, variable=var).pack(side=tk.LEFT)
            elif isinstance(var, tk.IntVar):
                tk.Spinbox(row_frame, from_=0, to=1000000, textvariable=var, width=10).pack(side=tk.LEFT)
            elif isinstance(var, tk.DoubleVar):
                tk.Entry(row_frame, textvariable=var, width=15).pack(side=tk.LEFT)
            else:
                width = 50 if len(str(var.get())) > 20 or "KEY" in key else 25
                tk.Entry(row_frame, textvariable=var, width=width).pack(side=tk.LEFT)
    ...
    def _add_spinbox(self, parent, label_text, var, min_val, max_val):
        ...
        tk.Spinbox(f, from_=min_val, to=max_val, textvariable=var, width=10).pack(side=tk.LEFT)
    ...
    def save_all_settings(self):
        """
        Saves all global and plugin-specific settings to their respective JSON files.
        """
        try:
            # 1. Prepare and Save Global Settings
            global_settings = {
            ...
            # 2. Reload the config module to reflect changes globally
            config.Config.reload()

            # 3. Save Plugin-Specific Settings (API Keys, etc.)
            all_plugins = self.extractors + self.validators
            for plugin in all_plugins:
                if plugin.name in self.plugin_vars:
                    settings_map = self.plugin_vars[plugin.name]
                    for key, var in settings_map.items():
                        plugin.config[key] = var.get()
                    plugin.save_settings()

            # 4. Trigger UI Refresh for Configuration Banner
